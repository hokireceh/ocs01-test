...
fn select_wallet(wallets: &[Wallet]) -> Result<usize> {
    println!("\n--- Select Wallet ---");
    for (i, wallet) in wallets.iter().enumerate() {
        println!("{}. {} ({})", i + 1, wallet.name, wallet.addr);
    }
    
    loop {
        let choice = read_input("\nSelect wallet (number): ");
        if let Ok(idx) = choice.parse::<usize>() {
            if idx > 0 && idx <= wallets.len() {
                return Ok(idx - 1);
            }
        }
...
fn load_wallets() -> Result<Vec<Wallet>> {
    // Try to load multi-wallet config first
    if let Ok(config_content) = fs::read_to_string("wallets.json") {
        let config: WalletConfig = serde_json::from_str(&config_content)?;
        return Ok(config.wallets);
    }
    
    // Fall back to single wallet.json for backward compatibility
    if let Ok(wallet_content) = fs::read_to_string("wallet.json") {
        let wallet: Wallet = serde_json::from_str(&wallet_content)?;
        return Ok(vec![wallet]);
    }
    
    bail!("No wallet configuration found. Please create wallets.json or wallet.json");
}
...
fn main() -> Result<()> {
    ...
    let mut current_wallet_idx = if wallets.len() > 1 {
        select_wallet(&wallets)?
    } else {
        0
    };
    
    loop {
        let wallet = &wallets[current_wallet_idx];
        ...
        if wallets.len() > 1 {
            println!("current wallet: {} ({})", wallet.name, wallet.addr);
        }
        ...
        if wallets.len() > 1 {
            println!("{}. switch wallet", interface.methods.len() + 1);
        }
        println!("0. exit");
        ...
        if let Ok(idx) = choice.parse::<usize>() {
            if wallets.len() > 1 && idx == interface.methods.len() + 1 {
                current_wallet_idx = select_wallet(&wallets)?;
                continue;
            }
